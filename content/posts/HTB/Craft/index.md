...
Front page suggest that we will work with some API calls. both menu options use two new subdoimains "api.craft.htb" and "gogs.craft.htb". I will add them to my `/etc/hosts` and also run `ffuf` to look for any other sub-domains and also to enumerate directories.
...
api subdomain hosts different api calls. Two interesting one are authentication check to check validity of an auhtorization token and the authentication login to create the said token.
...
I suspect there will be some API keys, tokens or creds in the repository by accident. I found a conversation regarding adding Bogus ABV values to the database, they patched it to some extend but from what I see and feel, it still isn't secure - this will be a great exercise to work on API calls and understand this part of development.
...
There are two interesting parts of the script for us. It uses `eval()` which is a known dangerous function in a number of different programming languages. It's dangerious because it runs string data as an executable instruction. 
The second interesting part is that `request.json['abv'])` plainly outputs uunfiltered user output into the command.
Both of these weakneses are bad on thier own as one gives a possibility of command execution and another of command injection. Together they are a really great foothold opportunity.

```
pride@parrot (~): curl -H 'X-Craft-API-Token: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoidXNlciIsImV4cCI6MTU0OTM4NTI0Mn0.-wW1aJkLQDOE-GP5pQd3z_BJTe2Uo0jJ_mQ238P5Dqw' -H "Content-Type: application/json" -k -X POST https://api.craft.htb/api/brew/ --data '{"name":"a","brewer":"a","style":"a","abv":"__import__(\"os\").system(\"id\")"}' 
...
I first tried to use mysql locally - but it isn't installed - and call it remotly with `mysql -u craft -pqLGockJ6G2J75O -h 10.129.22.88` - but this doesn't work and hangs my shell. This is becasue the database isn't local, it's in fact on the `db` host which I assume is the docker deamon.
Due to the fact that my shell is connected to a simple web request it's limited by the timeout time of the webserve which is proximitly 60 seconds. Due to that limitation, I was thinking how to best access the database. As my shell access was somewhat flimsy I didn't want to bother setting up a chisel tunnel and work with transfering files - which also ruled out downloading `mysql` and similar tooling.
What I stumbled upon was `pymysql` which is a python library for working with sql. As the whole box is somehow very Pythony for me from start until now, I decided to try it out.
...
Let's try to acces SSH with them.
...
Digging furhter into gilfoyle I found that he had a private repository called `craft-infra` on which we can find his public and private SSH keys, likely to the dc host.
``` id_rsa
pride@parrot (~/Desktop/htb/machines/craft): cat id_rsa 
...
When I try to authenticate with `ssh -i id_rsa gilfoyle@10.129.22.88` i get a message "Load key "id_rsa": error in libcrypto". From what I've read this can happen when SSH expects an older private key format called PEM. You can easly know which one is which by looking at the first line:
New one: `-----BEGIN OPENSSH PRIVATE KEY-----`
Old one: `-----BEGIN RSA PRIVATE KEY-----`

Luckly, the key can be formated easly with `ssh-keygen`. First lets make a copy of the original with `cp id_rsa id_rsa-original` and format the copy with `ssh-keygen -p -f id_rsa -m PEM`.
...
Enumerating the user they don't have any low hanging permissions or rights to take adventage on. Interestingly, I'm on the `craft.htb` host and not `db` which I suspected was the hostname of the docker host.
I looked a bit further and found `.vault-token` file which contains this token `f1783c8d-41c7-0b12-d1c1-cf2aa17ac6b9gilfoyle`. I looked throught the file system with `find / -iname "*vault*" 2>/dev/null` and found these files
``` 
...
I then ran looked through them manually and greped for key words in them but didn't find anythin interesting. I tried to ssh into the port 6022 as maybe that is the vault that is mentioned. The amount of SSH files suggested that but I still can't authenticate there.
...
I can read and list secrets from a vault, the issue is that I don't know the path to it. I tried to do `vault list /ssh/roles/root_otp` as I saw this path in `secrets.sh` - didin't work and seemed far fetched.
...
SSH using CA mode with host key verification:
	...
	user@example.com
```
There are three way to authenticate "one time password" and two "certificate authority" modes. Looking at the token I found it looks more like a OTP authentication.
...
The first command is the most important one - it creates a role `root_otp` which can request to get OTPs for root and those request can come from any IP. This represents a lazy admin setup and because of this can get a root access simpy by requesting it.
The token is a OTP that was used by the user, it showed me how it looks like.
...
This box was challanging to me, one of the most confusing I worked on. I had little expirience until now with workign with APIs and creating injections is something I need to practice more. I liked that I challanged myself to write my first working exploit for this box and it helped me to learn and refresh my Python knowledge. Simulation of reading up on a git repo and a really hands on code review was a great learning expirience. I never worked with HashiCorp Vault before so this was also interesting - a lot of pivoting as well.

For code review and injections I think is important to try to really concentrate, go down the rabbit hole and really try to understand the logic of the mechanism. Sounds trivial I know, but I feel I could save a lot of time by starting with such hard mindset from the begenning.

PS: I still didn't figure out what was port 6022 used for ¯\_(ツ)_/¯
